...
To conclude this section, we want to emphasize that increasing body stiffness has a significant impact on the evacuation dynamics. The stiffer pedestrians are, the less they touch to each other (less degree and less overlap). This yields a reduction of an increment of the speed, which reduces the evacuation time. \\
...
It is worth mentioning that increasing the body force reduces the flow, for a given density and a given friction value (compare \ref{flow_density_no_body_force} with \ref{flow_density_body_force} ). In other words, pedestrians walk slower along the corridor if the $k_n$ value increases (\textit{i.e.} if they are stiffer). This result is the opposite of what we get in bottleneck geometry since in bottleneck, the stiffer the pedestrians are, the faster they walk (and evacuate).\\
...
As was mentioned above, increasing the $k_n$ value of the body force increases the stiffness of pedestrians. Increasing the stiffness reduces the overlap (both in bottleneck and corridor). Regarding the network connectivity, increasing the stiffness reduces the mean degree in a bottleneck but increases the mean degree in a corridor. In Fig.~\ref{network_corridor} it shown the contact network corresponding to the corridor at a particular timestep and a particular global density. Fig.~\ref{network_d6_kn0} corresponds to $k_n=0$ while Fig. \ref{network_d6_knE5} corresponds to $k_n=1.2$~E5. This is an example to show that increasing the stiffness increases the number of contacts between pedestrians (the network connectivity). Notice that there are many more pedestrians with degree 6 in the case of $k_n=1.2$~E5 than the case of $k_n=0$. \\
