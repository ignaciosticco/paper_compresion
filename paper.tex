...
Increasing $k_n$ produces opposite effects on either geometry: an increase of the crowd velocity 
for bottlenecks, and a decrease for corridors. 
The former reflects the fact that the stiffness reduces
the overlaps and the sliding friction among pedestrians. 
This phenomenon reduces the number of blocking clusters close to the exit door. 
The corridor, instead, tights the pedestrians into a lattice-like configuration due to space limitations.
The friction interaction with the walls determines the velocity of the whole crowd along the corridor.
Additionally, the corridor geometry experiences a flux slowing down for very
