...
counterbalanced by the right \textcolor{blue}{\sout{setting} choice } of $A$ or $k_t$, keeping the 
...
konwledge). We are aware, though, of the drawbacks \textcolor{blue}{of} this choice, say, the 
...
attaining a \textit{privacy sphere} \textcolor{blue}{that excludes second neighbors}. The desired 
...
the corridor \textcolor{blue}{\sout{(at densities above} 
as the density increased with a sharp increase at} $5\,$p/m$^2$\textcolor{blue}{\sout{)}}. Conversely, the mean degree 
...
Fig.~\ref{overlap_dens}. \textcolor{red}{\sout{The curve meeting attains for space limitations in 
...
All the curves meet each other at high-density values. This phenomenon occurs
due to space limitations in the corridor that produces overlapping (almost) independently of the
value of $k_n$}. \\
...
The above results \textcolor{blue}{\sout{correspond to a milestone} represent an important step} in the 
investigation. The \textcolor{blue}{inter-pedestrian} connectivity differences between the bottleneck geometry 
and the corridor geometry were not expected and opens two major questions:
\textcolor{blue}{\sout{ how 
does the body stiffness interact with the walls} how do the pedestrians interact with the walls as
a function of the body stiffness}, and consequently, how does 
...
Fig.~\ref{overlap_dens},\textcolor{blue}{\sout{ these densities attain relevant overlaps}
...
behavior may be envisaged as the passage from a \textcolor{red}{\sout{released} ``free''} 
walking movement to a \textcolor{red}{\sout{tigh} constrained} movement as the stiffness increases. A more 
...
Fig.~\ref{vx_profile} shows the velocity profile ($\langle v_x \rangle$ vs. the transversal coordinate $y$) for different $k_n$ values. For $k_n=0$, we can see a parabolic-like velocity profile \textcolor{red}{which means that the friction with the walls reduces the speed of pedestrians.} \textcolor{blue}{\sout{that} The velocity profile} resembles the Poiseuille flow (similar to Newtonian and incompressible fluids in a laminar regime). This behavior was also observed in empirical measurements of crowd dynamics reported in Ref.~\cite{zhang2013empirical}. But as $k_n$ increases the velocity profile flattens until becoming (almost) uniform (see Fig.~\ref{vx_profile} for $k_n=1.2\,$E6). In this scenario, $v_x$ attains a much lower \textcolor{blue}{\sout{level} value}  than in the case of soft pedestrians ($k_n=$0).\\

\textcolor{blue}{From the results displayed above, we realize that \sout{The} the} crowd behaves like a solid for very stiff pedestrians. This means that the crowd can not be easily ``deformed''. In this context, deformation means that parts of the crowd may be \textcolor{blue}{\sout{released} allowed} to move faster than other parts of the crowd. \\

\textcolor{blue}{\sout{The strain rate is the tensor that expresses how the relative velocity of a medium changes when one moves away from one position to another.} The strain rate tensor displays the rate of change of the deformation of a body in the vincinity of a given point.} We consider the following discrete definition of the strain rate:
...
\textcolor{blue}{Having adopted as fundamental philosophy of work that the parameters of the SFM should be properly determined by requiring that the model properly reproduces well established experimental data, we} explored the effect on the pedestrians dynamics of the (sometimes neglected) body force in the framework of the SFM. We showed that the stiffness coefficient ($k_n$) has a significant impact on the evacuation dynamics (bottleneck) and also in the dynamics of pedestrians walking along a straight corridor.\\
...
Our efforts to ``tune'' the original SFM to \textcolor{blue}{reproduce} empirical data (say, the fundamental diagram) moved us to explore the dimensionless parameter space.  We found that \textcolor{blue}{\sout{the qualitative behavior of empirical  data can be accomplished} we can qualitatively reproduce the empirical data} if the parameters are close to $\mathcal{K}_c < 68$, $\mathcal{K} = 685$ and $\mathcal{A}=14$. Nevertheless, this is a first attempt to arrive at suitable parameters, although we do not claim these to be optimal. We encourage further research to find dimensionless numbers that may better fit experimental data.\\
