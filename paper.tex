...
The reason why the velocity reduces as the stiffness of the pedestrians increases is that it becomes harder for the pedestrians to walk forward since the repulsion force (body force) against the pedestrian that is ahead becoming higher. Thus, they have no other option than moving at (almost) the same speed than the pedestrians that they have at their sides.\\


\begin{figure}[htbp!]
\centering
\includegraphics[width=0.7\columnwidth]{./kn_vs_vx_corridor.eps}
\caption{\label{kn_vs_vx_corridor}   }
\end{figure}

The value of $k_n$ affects the velocity all along the corridor. Fig... shows the velocity profile ($v_x$ vs. the longitudinal coordinate $y$) for different $k_n$ values. For $k_n=0$, we can see a parabolic-like velocity profile that resembles the Poissele flow for a viscous liquid. As $k_n$ increases the velocity profile flattens until becoming a constant ($k_n=$ 1.2~E6) which $v_x$ value is much lower than on the case of soft pedestrians ($k_n=$0).\\

If the stiffness is high enough, the crowd behaves like a solid; this means that the crowd can not be deformed. This is express by the strain rate, which is shown in Fig.~\ref{}. \\


The strain rate is a quantity that characterizes the deformation of a physical object. It is defined as a tensor .... For this particular problem we define a discrete version of the strain rate that compares the velocity of the pedestrians close to the wall (boundary) and the pedestrians that are in the center of the corridor (center). \\
