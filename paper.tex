...
\textcolor{red}{to the compression of the human body}. \\

Fig.~\ref{vd_vs_te} shows the evacuation time as a function of the pedestrian's 
desired velocity \textcolor{red}{for different values of $k_n$}. The evacuation 
time is defined as the time until 80\% of the pedestrians \textcolor{red}{have} 
left the room. In this section we will focus on the evacuation time for 
\textcolor{red}{$2\,$m/s$<v_d<10\,$m/s}.\\
...
Ref.~\cite{anthropometric}, \textcolor{red}{empty} squares correspond 
to zero body force and circles correspond to an extreme value of stiffness (one 
order of magnitude higher than the original SFM). The \textcolor{red}{empty} 
...
\textcolor{red}{The region in which the slope is positive} means that the 
harder the pedestrians try to get out (higher $v_d$), the 
\textcolor{red}{longer} it takes them to evacuate. This is the Faster-is-slower 
(FIS) regime. Conversely, \textcolor{red}{the region in which the slope is 
negative}  corresponds to a Faster-is-Faster (FIF) \textcolor{red}{regime} (the 
harder they try, the quicker they leave). Fig.~\ref{vd_vs_te} shows a change in 
\textcolor{red}{in the behavior for desired velocities $v_d>1.2\,$m/s. The 
...
behavioral change.  Notice that \textcolor{red}{higher values of $k_n$ allow 
...
than, for example, $v_d=3\,$m/s because of the pedestrians \textcolor{red}{that 
...
significant \textcolor{red}{intrusion} of the personal space of the individuals 
(this means more overlap between pedestrians). \textcolor{red}{\sout{The 
increment in mean overlap, however, seems to be more significant than the mean 
degree.}} \\  


Increasing \textcolor{red}{the} body stiffness has a significant impact on 
evacuation dynamics. The reduction of the overlap and the mean degree between 
pedestrians \textcolor{red}{in the simulation}, tend to diminish the sliding 
...
Despite \textcolor{red}{the effects on the} sliding friction, the body force 
...
In this research, we have explored the effects of the body force in the original SFM. We showed that the stiffness coefficient ($k_n$) has a significant impact on the evacuation dynamics (bottleneck) and also in the dynamics of pedestrians walking along a straight corridor.\\

In the bottleneck geometry, the evacuation time diminishes (pedestrians move faster) as pedestrians become stiffer for all the ranges of the desired velocities explored. This phenomenon happens because increasing the stiffness level promotes the repulsion between pedestrians pair interactions. As pedestrians detach from each other, the friction force diminishes due to the reduction of overlap and contacts among them. The general decrease in frictional force reduces the probability of producing a cluster of pedestrians blocking the exist (blocking cluster), which finally leads to a more efficient evacuation dynamics. \\

In the corridor geometry, the walking speed reduces as pedestrians become stiffer. This is the opposite behavior we obtained in bottleneck geometry. The difference is explained because although increasing stiffness further separates individuals, they can not detach from each other as much as possible due to the lateral walls (these walls are not relevant in the bottleneck geometry). Thus, when the stiffness increases, pedestrians reduce the overlap but increase the number of contacts (degree). In high density scenarios, the stiffness promotes the friction force.  Granular materials can behave like fluid or solids, depending on several conditions. If the stiffness level is low, the entire crowd behaves like a viscous fluid (with a parabolic velocity profile). However, if the stiffness level is high enough, the whole crowd behaves like a solid (with a constant velocity profile which depends on the friction against the walls).\\
