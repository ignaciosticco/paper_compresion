...
We computed the contact network in the same way as in the bottleneck geometry. Fig.~\ref{degree_dens} shows the mean degree as a function of the global density for different $k_n$ values (see caption for details). The degree is zero when the pedestrians do not touch each other. When the density reaches 4.5, a few pedestrians start to touch each other increasing the mean degree. As the density continuous increasing, the mean degree increases towards the asymptotic degree six. Degree six corresponds to the maximum packing density for identical hard circles.\\
