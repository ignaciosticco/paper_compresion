...
Our efforts to ``tune'' the original SFM to \textcolor{blue}{reproduce} empirical data (say, the fundamental diagram) moved us to explore the dimensionless parameter space.  We found that \textcolor{blue}{\sout{the qualitative behavior of empirical  data can be accomplished} we can qualitatively reproduce the empirical data} if the parameters are close to $\mathcal{•}hcal{K}_c < 68$, $\mathcal{K} = 685$ and $\mathcal{A}=14$. Nevertheless, this is a first attempt to arrive at suitable parameters, although we do not claim these to be optimal. We encourage further research to find dimensionless numbers that may better fit experimental data.\\
